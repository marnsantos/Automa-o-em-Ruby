# Esta classe representa a tela Cadastro de Empresas
class CadastrarEmpresa < SitePrism::Page
  set_url '/company-list'

  element :botao_novo, '#new-employee'
  ...
  element :botao_cancelar, 'button.btn-default'
    ...
    def cadastrar_empresa(cidade, nome, cnpj, url, telefone, contato, rua, numero, bairro, cep, complemento)
    ...
    def cancelar_operacao(cidade, nome, cnpj, url, telefone, contato, rua, numero, bairro, cep, complemento)
        Capybara.default_max_wait_time = 3.to_i
        botao_novo.click
        campo_cidade.click
        campo_cidade.find(:xpath, '//span[text()=' + "'" + cidade + "'" + ']').select_option
